...
public struct CloudReceiverView: View {
    ...
    public var body: some View {
        HSplitView {
            // Left Pane: Map
            WorldMapView(messages: $ft8Engine.decodedMessages)
                .frame(minWidth: 400, idealWidth: 600)
                .layoutPriority(1)  // Encourage map to take space

            // Right Pane: Controls & Decoder
            VStack(spacing: 0) {
                // Header / Controls
                HStack {
                    ...
                    Button(kiwiClient.isConnected ? "Disconnect" : "Connect") {
                        if kiwiClient.isConnected {
                            ...
                            print("CloudReceiverView: Connecting to \(rec.0)")
